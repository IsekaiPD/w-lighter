...
def work_create(request):
    ...
    errors = {}
    if not title:
        errors['title'] = '작품 제목을 입력해주세요'
    if not pen_name:
        errors['author'] = '필명을 입력해주세요'
    if not genre:
        errors['genre'] = '장르를 선택해주세요'
...
def _episode_number(episode):
    return Episode.objects.filter(
        work=episode.work,
        episode_id__lte=episode.episode_id,
    ).count()
...
def episode_edit(request, work_pk, episode_pk):
    ...
    if request.method == 'POST':
        ...
        errors = {}
        if not title:
            errors['title'] = '회차 제목을 입력해주세요'
        if not original_text:
            errors['content'] = '원문을 입력해주세요'
        if errors:
            return JsonResponse({'ok': False, 'errors': errors})

        episode.title         = title
        episode.original_text = original_text
        episode.save()
        return JsonResponse({'ok': True})
...
def episode_register(request, work_pk):
    ...
    return render(request, 'works/episode_register.html', {'work': work})
